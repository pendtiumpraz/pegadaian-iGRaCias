...
export default function LossCreate({ users = [] }) {
    const { data, setData, post, processing, errors } = useForm({
        nomor_loss:        '',
        judul:             '',
        deskripsi:         '',
        kategori:          'internal_fraud',
        tanggal_kejadian:  '',
        jumlah_kerugian:   '',
        mata_uang:         'IDR',
        status:            'identified',
    ...
    function submit(e) {
        e.preventDefault()
        post(route('loss.store'))
    }

    function handleAIChip(chip) {
        // Placeholder: chip click could prefill kategori based on text match.
    ...
    return (
        <AppLayout title="Catat Loss Event" contentPadding="none">
            <PageHeader
                title="Catat Loss Event"
                description="Dokumentasikan kejadian kerugian operasional baru sesuai taksonomi Basel."
                breadcrumbs={[
            ...
            <div style={{ padding: '20px 32px' }}>
                ...
                <div style={{ marginBottom: 14 }}>
                    <AIGradientBanner
                        ...
                        body="Tulis kronologi atau deskripsi pada form di bawah, lalu pilih saran kategori Basel yang sesuai."
                        chips={AI_CHIPS}
                ...
                <form onSubmit={submit}>
                    ...
                    <div style={cardStyle}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                            <div style={grid2}>
                                <Field label="Nomor Loss" error={errors.nomor_loss} hint="Mis. LOSS-2026-001">
                                    <input
                                        style={inputMonoStyle}
                                        value={data.nomor_loss}
                                        onChange={e => setData('nomor_loss', e.target.value)}
                                        placeholder="LOSS-2026-001"
                                    />
                                </Field>
                    ...
                    {/* Nilai & Status */}
                    <div style={cardStyle}>
                        <SectionTitle title="Nilai & Status" hint="Jumlah kerugian dan tahap penanganan" />
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                            <div style={grid3}>
                    ...
                    <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 18 }}>
                        <Link href={route('loss.index')} style={btnSecondary}>Batal</Link>
                        <button type="submit" disabled={processing} style={{ ...btnPrimary, opacity: processing ? 0.6 : 1 }}>
                            {processing ? 'Menyimpan…' : 'Simpan Loss Event'}
                        </button>
                    </div>
                </form>
            </div>
